...
- **Linux / WSL2**
  ...
  - **libwebp**（SOG 格式的 WebP 纹理编解码，运行时动态加载，缺少时程序仍可启动但 SOG 功能不可用）：
...
- **Windows**
  ...
  - CMake 3.20+
  - CUDA Toolkit 11.0+
  - **libwebp**（可选，同上）：推荐通过 [vcpkg](https://github.com/microsoft/vcpkg) 安装，或手动将 `libwebp.dll` 放至可执行文件同级目录：
    ...
    `libwebp.dll` 默认通常位于
    `C:\src\vcpkg\installed\x64-windows\bin\libwebp.dll`。

    OptiSplat 在 Windows 上是运行时动态加载 `libwebp.dll`，因此“已经通过
    `vcpkg` 安装 libwebp”并不等于“当前 VS Code / 终端 / 调试器进程一定能找到
    这个 DLL”。

    推荐做法：
    1. 最稳妥的是直接把 `libwebp.dll` 复制到可执行文件同级目录，例如
       `build\Release`。
    2. 如果希望通过 `PATH` 使用，请在修改系统环境变量后完全重启 VS Code /
       终端 / 调试器，或重新登录 Windows，让新的环境变量被继承。
    3. 如果要临时验证，可在 PowerShell 中执行：

    ```powershell
    $env:PATH = "C:\src\vcpkg\installed\x64-windows\bin;$env:PATH"
    .\build\Release\optisplat_test.exe
    ```
