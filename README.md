...
- **🚀 高性能渲染**
  - FlashGS 精确相交检测 — 优化预处理 (preprocess) 阶段和预取流水线（renderCUDA）阶段
...
2. 构建产物：
   - 可执行文件：`./optisplat_test`
   - （可选）如果只关心 Python 使用，也可以仅执行 `pip install -e .` 来构建扩展模块。

## 🚀 用法

### C++ 示例

编辑 `src/main.cc` 配置模型和相机路径：
